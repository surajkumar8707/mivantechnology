...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <form action="{{ $route }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label class="form-label">Image {{ isset($gallery) ? '(leave blank to keep current)' : '' }}</label>
            ...
            <div class="mb-3">
                ...
                <select name="type" class="form-control">
                    <option value="mivan" {{ old('type', $gallery->type ?? '') == 'mivan' ? 'selected' : '' }}>Mivan
                    </option>
                    <option value="post_tensioning"
                        {{ old('type', $gallery->type ?? '') == 'post_tensioning' ? 'selected' : '' }}>Post Tensioning
                    </option>
                </select>
            </div>
